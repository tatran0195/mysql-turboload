...
**Date**: 2026-09-18  
**Topic**: Persistent TOML Configuration Loading, CLI Merging Hierarchy, and Unified Directory Option for MySQL TurboLoad  
...
Currently, `mysql-turboload` requires operators to pass all database connection parameters, performance tuning options, worker concurrency counts, and directory paths via CLI flags or environment variables for every execution. In enterprise database operations:
...
2. **Config Discovery & Loading Engine**:
   ...
   * Automatic hierarchical candidate search:
     ...
     3. User platform configuration directory (`%APPDATA%\mysql-turboload\config.toml` on Windows, `~/.config/mysql-turboload/config.toml` on Linux/macOS).
...
### 3.1 TOML Structure (`turboload.toml`)
```toml
[connection]
host = "127.0.0.1"
port = 3306
...
pub fn resolve_config_path(explicit: Option<&Path>) -> Option<PathBuf> {
    ...
    if let Some(config_dir) = dirs_next_or_custom() {
        let user_cfg = config_dir.join("mysql-turboload").join("config.toml");
        if user_cfg.is_file() {
...
When a configuration file is successfully discovered and parsed, `mysql-turboload` prints a status message:
```text
[CONFIG] Loaded settings from: ./turboload.toml
```
...
2. **Precedence & Merge Tests**:
   ...
   * `--no-config` prevents loading even if `turboload.toml` exists in the filesystem.
   * Explicit `--config missing.toml` produces a clear descriptive error.
3. **Unified `--dir` Verification**:
   * `mysql-turboload export -d ./custom_dumps` properly sets `dir`.
   * CLI help output for `mysql-turboload export --help` reflects `-d, --dir <DIR>`.
4. **End-to-End Dry-Run Tests**:
   * Run with `--dry-run` and a sample `turboload.toml` to verify printed banner and runtime settings reflect TOML values.
