...
In `mysql-turboload`, the export engine (`exporter.rs`) currently outputs manifests and error logs into a subdirectory named `import-logs` (e.g. `./dumps\import-logs\manifest.json`). This was a leftover naming convention from the original import tool. Additionally, the import CLI defaulted `--log-dir` to `import-logs`.
