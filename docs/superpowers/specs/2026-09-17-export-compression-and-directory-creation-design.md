...
1. **Target Directory Handling**:
   - Ensure `mysql-turboload export` supports `-d` and `--dir` as aliases for `-o` / `--output-dir`.
   - Prevent falling back to the default `"export-dumps"` directory if a directory was specified at the root command level (e.g. `mysql-turboload -d ./dumps export`).
...
2. **Streaming Export Compression (`--compress` / `-z`)**:
   ...
   - Include standard Workbench / TurboLoad header directly inside the compressed archive.
3. **Import Round-Trip Decompression**:
   - Update `mysql-turboload import` to discover both `.sql` and `.sql.gz` dump files.
...
- Add unit tests verifying:
  - `mysql-turboload export -d ./target` sets `output_dir` to `./target`.
  - `mysql-turboload export --dir ./target` sets `output_dir` to `./target`.
  - `mysql-turboload export -o ./target` sets `output_dir` to `./target`.
  - `mysql-turboload export --compress` and `export -z` enable `compress: true`.
