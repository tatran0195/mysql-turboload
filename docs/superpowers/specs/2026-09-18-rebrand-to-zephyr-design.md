...
**Date**: 2026-09-18  
**Topic**: Comprehensive Rebrand from `zephyr` to `zephyr` ("Zephyr: High-Performance Concurrent MySQL Data Engine")  
...
The project began under the name `zephyr` as a specialized bulk ingestion engine for MySQL database dumps. Over recent releases, the tool has evolved into a complete, bidirectional high-throughput engine providing:
...
- **Misleading Scope**: The name "Zephyr" implies an ingestion-only ("load") utility, neglecting its enterprise-grade export capabilities.
- **CLI Ergonomics**: Typing `zephyr` repeatedly in terminal workflows and automation scripts is verbose and cumbersome.
...
- **Product Name**: **Zephyr**
- **Tagline**: _"High-Performance Concurrent MySQL Data Engine"_
...
- Rename crate package from `zephyr` to `zephyr`.
- Update description to `"Zephyr: High-performance concurrent MySQL bulk data engine"`.
- Update binary target `[[bin]]` from `name = "zephyr"` to `name = "zephyr"`.
...
- Update mock invocation strings in CLI unit tests from `"zephyr"` to `"zephyr"`.

### 3.3 Configuration System (`src/config.rs`)

- Rename struct `ZephyrConfig` $\rightarrow$ `ZephyrConfig`.
- Update automatic config discovery candidate order in `find_config_file()`:
    ...
    3. `./mysql-zephyr.toml`
    4. OS AppData / Home config paths:
...
- Update `env!("CARGO_BIN_EXE_zephyr")` references to `env!("CARGO_BIN_EXE_zephyr")`.
...
1. **Config File Fallbacks**: Operators with existing `zephyr.toml` or `zephyr.toml` files will continue to have their configurations automatically loaded if `zephyr.toml` is not present.
