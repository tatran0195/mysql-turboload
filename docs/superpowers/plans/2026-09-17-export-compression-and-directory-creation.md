...
**Goal:** Add streaming Gzip compression (`--compress` / `-z`) to `zephyr export`, support `-d` and `--dir` aliases with recursive directory auto-creation, and enable transparent `.sql.gz` decompression during `zephyr import`.
...
- [ ] **Step 2: Run test to verify it fails**
      Run: `cargo test --bin zephyr test_export_args_dir_aliases`
...
- [ ] **Step 4: Run test to verify it passes**
      Run: `cargo test --bin zephyr test_export_args_`
